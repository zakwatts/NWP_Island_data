...
def create_ds(path: Path) -> xr.Dataset:
    ...
    ds_t = xr.open_dataset(
        path,
        #I have changed the engine path from cfgrib to zarr
        engine="cfgrib",
        backend_kwargs={
            "filter_by_keys": {
                "cfVarName": "t",
    ...
    ds_mcl = ds_mcl.rename({'tcc': 'tcc_middle_cloud'})
    ...
    ds_hcl = ds_hcl.rename({'tcc': 'tcc_high_cloud'})
    """
    ...
    ds_ccb = xr.open_dataset(

    ds_blcl = xr.open_dataset(

    dsntcl = xr.open_dataset(
    """
    ...
    ds_merged = xr.merge([ds_t, ds_u, ds_v, ds_d, ds_p, ds_dswrf, ds_hum, ds_mcl, ds_hcl],compat='override')
